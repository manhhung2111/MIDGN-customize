...
def graph_generating(raw_graph, row, col):
    # The sp.bmat function creates a sparse matrix by combining smaller sparse matrices into a larger block matrix:
    # - The top-left block of the graph matrix is an identity matrix of size row (square matrix).
    # - The top-right block of the graph matrix is the raw_graph matrix.
    # - The bottom-left block is the transpose of raw_graph.
    # - The bottom-right block is an identity matrix of size col (square matrix).
...
class MIDGN(Model):
    ...
    def __init__(self, info, dataset, raw_graph, device, pretrain=None):
        ...
        emb_dim = int(int(self.embedding_size) / self.n_factors)  # Embedding size of each trunk
        self.items_feature_each = nn.Parameter(
            torch.FloatTensor(self.num_items, emb_dim)).to(device)
        nn.init.xavier_normal_(self.items_feature_each)  # Random parameters
        ...
        ui_e_indices, ui_e_values = torch_sparse.spspmm(ub_indices, ub_values, bi_indices, bi_values, self.num_users,
                                                        self.num_bundles,
                                                        self.num_items)  # sparse matrix multiplication

        ui_graph_e = sp.csr_matrix(
            (np.array([1] * len(ui_e_values)), (ui_e_indices[0].numpy(), ui_e_indices[1].numpy())),
        ...
        # Construct ui_graph and bi_graph
